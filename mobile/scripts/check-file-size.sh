...
while IFS= read -r -d '' file; do
  if ! awk -v file="$file" -v max_file_lines="$max_file_lines" -v max_fn_lines="$max_fn_lines" '
    ...
    function strip_line_comments(line,   out, i, ch, next_ch, in_str, esc) {
      out = ""
      in_str = 0
      esc = 0
    ...
    function count_braces(line,   cleaned, i, ch, in_str, esc, c) {
      cleaned = strip_line_comments(line)
      c = 0
      in_str = 0
      esc = 0
