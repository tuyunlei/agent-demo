...
❌ Agent 不会用工具 — 只能纯文本聊天，不能搜索、不能查时间
❌ Agent 没有时间感知 — 不知道消息是什么时候发的
...
让 agent 从"转发器"变成"能做事的助手"：

| 顺序 | 功能 | 用户感知 | 状态 |
|------|------|----------|------|
| 1 | 工具调用链路 + get_current_time | 问"现在几点"，agent 能调工具回答 | 🔲 |
| 2 | web_search 工具 | 问"帮我搜 XXX"，agent 能搜索并总结 | 🔲 |
| 3 | 上下文时间戳 + System Prompt 增强 | agent 知道每条消息的时间，能说"你 2 小时前提到过" | 🔲 |
...
| 顺序 | 功能 | 用户感知 | 状态 |
|------|------|----------|------|
| 4 | 多会话管理 | 能新建对话、切换对话、看到会话列表 | 🔲 |
| 5 | Token 自动刷新 | 不会突然被踢出去重新登录 | 🔲 |
| 6 | 统一错误处理 | 出错时能看到有意义的提示 | 🔲 |
